...
def slice_fit(array, width, axis): #gives a gaussian fit of slice of width w, ignoring any of the saturated data points
    zeros = np.where(array>0.9, 1, 0)
    if axis=='x':
        zeros = np.unique(np.nonzero(zeros)[1])
        slice = center_data(array, width)[0]
        data = proj_norm(slice)[0]
    if axis=='y':
        zeros = np.unique(np.nonzero(zeros)[0])
        slice = center_data(array, width)[1]
        data = proj_norm(slice)[1]
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        #adds lines to show slice locations
        center = img_center(a1)
        beam_plot.axvline(x=center[0]-width/2, color='red')
        beam_plot.axvline(x=center[0]+width/2, color='red')
        beam_plot.axhline(y=center[1]-width/2, color='red')
        beam_plot.axhline(y=center[1]+width/2, color='red')
        #labels the axis
        ...
        xProj = divider.append_axes("top", 1.8, pad=0, sharex=beam_plot)
        xProj.xaxis.set_tick_params(labelbottom=False)
        
        yProj = divider.append_axes("right", 1.8, pad=0, sharey=beam_plot)
        yProj.yaxis.set_tick_params(labelleft=False)
        
        #creates plot for the x projection
        xProj.margins(0, 0.1)
        xProj.yaxis.set_major_formatter(plt.NullFormatter())
        xProj.yaxis.set_major_locator(plt.MaxNLocator(6))
        #plots main functions
        xProj.plot(xIndex, xFit, label='gaussian fit', linewidth=2.5, color='black')
        xProj.scatter(xIndex, xData, label='raw data', s=25)
        xProj.scatter(xSlice[2], xSlice[3], label='fit data', s=25)
        #plots the saturation point
        xProj.axhline(y=1, label='saturation point', linewidth=1.5, color='red')
        #sets up legend
        ...
        yProj.margins(0.1, 0)
        yProj.xaxis.set_major_formatter(plt.NullFormatter())
        yProj.xaxis.set_major_locator(plt.MaxNLocator(6))
        #plots main functions
        yProj.plot(yFit, yIndex, label='gaussian fit', linewidth=2.5, color='black')
        yProj.scatter(yData, yIndex, label='raw data', s=25)
        yProj.scatter(ySlice[3], ySlice[2], label='fit data', s=25)
        #plots the saturation point
        yProj.axvline(x=1, label='saturation point', linewidth=1.5, color='red')
        #sets up legend
        ...
        l6 = QLabel(self); l6.setText(''.join(["Slice Width = ", str(round(width*1.12,1))," \u03BCm" ]))
        ...
        grid2.addWidget(l6, 6, 0, alignment=Qt.AlignTop | Qt.AlignLeft)
